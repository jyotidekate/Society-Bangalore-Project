...
import com.society.application.repository.NewSavingAccountRepo;
import com.society.application.repository.NewSchemeRepo;
...
public class SavingAccountController {
	...
	// Savings -> New Account 
	@GetMapping("/newAccountInSaving")
	public String newAccountInSaving(Model model) {
		long accNo = newSavingAccountRepo.countOfaccountNo();
		long accountNo = accNo + 1;
		model.addAttribute("accountNo", accountNo);
		return "savingsAccount/NewAccount";
	}
	
	@PostMapping("/fetchDataByClientNoInNewSavingAccount")
	@ResponseBody
	public List<ClientMaster> fetchDataByClientNoInNewSavingAccount(@RequestBody ClientMaster clientMaster){
	    List<ClientMaster> list = clientMasterRepo.findByclientNo(clientMaster.getClientNo());
	    return list;
	}

	@PostMapping("/fetchAllSchema")
	@ResponseBody
	public List<NewScheme> fetchAllSchema(){
		return newSchemeRepo.findAll();
	}
	
	@PostMapping("/fetchDataBySchema")
	@ResponseBody
	public List<NewScheme> fetchDataBySchema(@RequestBody NewScheme newScheme){
		return newSchemeRepo.findByschemeName(newScheme.getSchemeName());
	}
	
	/*@PostMapping("/saveingNewSavingAccountModule")
	@ResponseBody
	public ResponseEntity<String> saveingNewSavingAccountModule(@RequestBody NewSavingAccount newSavingAccount) {
	    try {
	        newSavingAccount.setFlag("1");
	        newSavingAccountRepo.save(newSavingAccount);
	        String successMessage = "Successful Account No. " + newSavingAccount.getGlHeadNo() + "/" + newSavingAccount.getAccountNo();
	 ...
	 * saveingNewSavingAccountModule(@RequestBody NewSavingAccount newSavingAccount)
	 * { try { // Check if a saving account already exists for the client boolean
	 * savingAccountExists =
	 * newSavingAccountRepo.existsByClientNo(newSavingAccount.getClientNo()); if
	 * (savingAccountExists) { return ResponseEntity.status(HttpStatus.BAD_REQUEST)
	 * .body("Saving Account already exists for Client No. : " +
	 * newSavingAccount.getClientNo() + " & Client Name : " +
	 * newSavingAccount.getClientName()); }
	 * 
	 * newSavingAccount.setFlag("1"); newSavingAccountRepo.save(newSavingAccount);
	 * String successMessage = "Successful Account No. " +
	 * newSavingAccount.getGlHeadNo() + "/" + newSavingAccount.getAccountNo();
	 * return ResponseEntity.ok(successMessage); } catch (Exception e) {
	...
	public ResponseEntity<String> saveingNewSavingAccountModule(@RequestBody NewSavingAccount newSavingAccount) {
	    try {
	        if (newSavingAccount.getGlHeadNo() == null) {
	            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("glHeadNo cannot be null");
	        }

	        boolean savingAccountExists = newSavingAccountRepo.existsByClientNo(newSavingAccount.getClientNo());
	        if (savingAccountExists) {
	            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
	                    .body("Saving Account already exists for Client No. : " + newSavingAccount.getClientNo() +
	                            " & Client Name : " + newSavingAccount.getClientName());
	        }

	        newSavingAccount.setFlag("1");
	        newSavingAccountRepo.save(newSavingAccount);
	        String successMessage = "Successful Account No. " + newSavingAccount.getGlHeadNo() + "/"
	                + newSavingAccount.getAccountNo();
	        return ResponseEntity.ok(successMessage);
	    } catch (Exception e) {
	        e.printStackTrace();
	        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error processing the request.");
